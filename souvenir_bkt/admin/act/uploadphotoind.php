<?php 
	include ('../../../connect.php');
	$id = $_POST['id'];
	$querysearch="select serial_number from industry_gallery where id='$id' order by serial_number desc limit 1";
	...
	if(($jenis_gambar=="image/jpeg" || $jenis_gambar=="image/jpg" || $jenis_gambar=="image/gif"  || $jenis_gambar=="image/png") && ($_FILES["image"]["size"] <= 5000000)){
		...
		$sql = pg_query("insert into industry_gallery values('$serial_number','$id','$name')");
		if($sql){
		...
		echo "Go Back To <a href='../?page=detailcraft&id=$id'>halaman detail</a>";
	}
?>